...
class PetCardListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val petListRecyclerView = binding.petlistrecyclerview
        ...
        viewModel.status.observe(viewLifecycleOwner) {status ->
            when (status){
                is PetCardUiState.Error -> Log.d("Pet List Status", "Error",status.exception)
                    PetCardUiState.Loading -> Log.d("Pet List Status", "Loading")
                is PetCardUiState.Success -> handleSuccess(status, petCardAdapter)
            }
        }
    }

    private fun handleSuccess(status: PetCardUiState.Success, petAdapter: PetCardAdapter) {
        status.pet.observe(viewLifecycleOwner){data ->
            petAdapter.setData(data)
        }
    }
}